...
[H-C CLOSED 2026-06-21 — hc_deep_subspace] Deep-feature (autoencoder latent) geometry: deep features DONT help over raw (latent_CVA 0.98 = raw_CVA 0.98); latent SUBSPACE (0.67) redundant/worse than latent CORRELATION (0.89). Geometry adds nothing on deep features (caveat: AE not a foundation model, but the subspace-vs-correlation redundancy holds regardless of feature source). ALL THREE planned leads (Bet1, Bet2, H-C) now CLOSED. CAMPAIGN COMPLETE: subspace geometry redundant/worse than a simpler standard statistic in EVERY tested cell. THE CONTRIBUTION = the DIAGNOSTIC. Next: consolidate diagnostic + seminar. Report: hc_deep_subspace_2026-06-21.
