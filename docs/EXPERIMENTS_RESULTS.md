...
|---|---|---|---|---|
| Satellite region-subspace proof | does a tile/object/region represented as a subspace of local satellite features add value over averaging? | AlphaEarth/EuroSAT/BigEarthNet/OpenEarthMap-style region task before change/anomaly triage | mean-pooled features, raw/foundation feature vector, linear probe, non-subspace distance, geographic split | subspace geometry improves low-label accuracy, retrieval, calibration, or cross-region robustness at equal feature source |
| Cross-region representation gate | does subspace representation survive geography shift better than pooled vectors? | disjoint-region land-cover/scene/retrieval task | random split baseline, geographic split, mean-vector classifier, shallow classifier | subspace loses less performance under geographic split or gives better few-label retrieval |
| Multimodal region-subspace gate | does a region subspace over optical/SAR or multimodal features preserve useful variation? | BigEarthNet-MM/OpenEarthMap-SAR-style task | concatenated mean vectors, modality-specific mean vectors, shallow classifier | subspace/product-subspace gives useful low-label or retrieval gain without hiding behind feature strength |
