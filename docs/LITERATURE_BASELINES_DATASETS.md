...
| AlphaEarth / Google Satellite Embeddings | Google Earth Engine `GOOGLE/SATELLITE_EMBEDDING/V1/ANNUAL`; AlphaEarth Foundations paper | 10 m annual 64-D pixel embeddings are both a strong baseline and a possible input feature field for Satellite Latent Subspace experiments |
...
| AlphaEarth Foundations / Satellite Embedding V1 | Google DeepMind and Earth Engine release; 64-D annual embeddings at 10 m resolution, designed for clustering, classification, and change detection | Directly overlaps with our proposed object. It already gives per-pixel learned EO embeddings, so our question becomes whether a region-level subspace of AlphaEarth/local embeddings adds value beyond pixel/mean/vector comparison. |
