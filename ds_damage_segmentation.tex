...
channels and fusion branches. On OSCD, raw Sentinel--2 segmentation is a
strong baseline, and adding a DS projection prior improves mean IoU/F1 in our
main 150--epoch run; fusion models can improve ranking metrics (AUROC) while
priors serve as valuable explanatory overlays. The code and design are structured so that the
...
cosine learning rate schedule. In the main run set, we train for 150 epochs
on GPU (single seed) and report mean metrics on the OSCD test split.
...
\begin{itemize}
  \item U--Net (raw pre+post): mean IoU \(0.223\), mean F1 \(0.343\),
  AUROC \(0.869\) (PR--AUC \(0.431\)).
  \item ResNet--U--Net (raw pre+post): mean IoU \(0.243\), mean F1 \(0.360\),
  AUROC \(0.800\) (PR--AUC \(0.393\)).
\end{itemize}

When change priors are added as extra channels:
\begin{itemize}
  \item DS projection prior improves the U--Net baseline:
  mean IoU \(0.273\), mean F1 \(0.401\), AUROC \(0.874\).
  \item Celik prior is also strong:
  mean IoU \(0.260\), mean F1 \(0.382\), AUROC \(0.872\) (best PR--AUC \(0.448\)).
  \item PriorsFusionUNet yields the highest AUROC in this run (\(0.888\)),
  but does not surpass the best U--Net + DS prior in IoU/F1.
  \item Several alternative priors hurt IoU/F1 in this run (pixel diff,
  PCA--diff, DS cross--residual, IR--MAD).
\end{itemize}
...
\begin{enumerate}
  ...
  than raw pixel diff.
  \item Phase~2 shows that supervised segmentation on raw Sentinel--2 is
  strong, and that DS projection priors can improve mean IoU/F1 in the current
  setup, while fusion can improve ranking metrics (AUROC/PR--AUC) even when it
  is not best for IoU/F1 at a fixed threshold.
\end{enumerate}
