...
void SetCLBPLAC(double *mat, int height, int width, int depth, int b, int bt, int r, int rt, double* extract){
    
    #define mat(y, x, z) (mat[(z)*width*height+(x)*height+(y)])   
    #define tmp(y, x, z, b) (tmp[(z)*width*height*8+(x)*height*8+(y)*8+(b)])  
    
    double* tmp = (double*)malloc(6*(height-2*b)*(width-2*b)*(depth-2*bt)*sizeof(double));
    
    for(int ti = bt;ti < depth-bt; ti++)
    for(int xi = b; xi < width-b; xi++)
    for(int yi = b; yi < height-b; yi++){
        double tmp_center = mat(yi,xi,ti);
        tmp(yi-b,xi-b,ti-bt,0) = tmp_center > mat(yi-b, xi  , ti   ) ? 1 : 0;
        tmp(yi-b,xi-b,ti-bt,1) = tmp_center > mat(yi+b, xi  , ti   ) ? 1 : 0;
        tmp(yi-b,xi-b,ti-bt,2) = tmp_center > mat(yi  , xi-b, ti   ) ? 1 : 0;
        tmp(yi-b,xi-b,ti-bt,3) = tmp_center > mat(yi  , xi+b, ti   ) ? 1 : 0;
        tmp(yi-b,xi-b,ti-bt,4) = tmp_center > mat(yi  , xi  , ti-bt) ? 1 : 0;
        tmp(yi-b,xi-b,ti-bt,5) = tmp_center > mat(yi  , xi  , ti+bt) ? 1 : 0;
    }
...
void mexFunction( int Nreturned, mxArray *returned[], int Noperand, const mxArray *operand[] ){
    double *x, *y;
    double *mat_0order, *mat_1order;
    double *extract;
    int rows, cols;
    int ClbplacDim = 256;
    int r, rt, b, bt;
    
    const int *Dim = mxGetDimensions(*operand);
    
    /* Matlab側に渡されるデータ(戻り値みたいなもの)を作る．C言語のmalloc()みたいなもの．        */
    /* Matlabの基本データ型は行列なので，行の数(rows)，列の数(cols)，要素が何か(mxREAL)を伝える */
    *returned = mxCreateDoubleMatrix(ClbplacDim, 1, mxREAL);
    
    
    /* Matlab側の変数のアドレスをC側のポインタにコピーする */
    mat = mxGetPr(operand[0]);
    extract = mxGetPr(*returned);
    b = (int)(*mxGetPr(operand[1]));
    bt= (int)(*mxGetPr(operand[2]));
    r = (int)(*mxGetPr(operand[3]));
    rt= (int)(*mxGetPr(operand[4]));
    
    
    //エラー処理
    if (mxGetNumberOfDimensions(*operand) != 5){
        printf("Error : The size of Matrix is incorrect\n");
        return;
    }
    if(b  < 1 ){
        printf("Error : b is incorrect\n");
        return;
    }
    if(bt < 1 ){
        printf("Error : bt is incorrect\n");
        return;
    }
    if(r  < 1 ){
    ...
    SetCLBPLAC(mat, Dim[0], Dim[1], Dim[2], b, bt, r, rt, extract);
}
