function [F1 F0] = cvtLBPLAC_Gabor(img, s, r, num_block_h, num_block_w)
% 
% %% 5 78 151 224
% 
if nargin < 4
...
if num_img  ~= 4
    gimg = zeros(h, w, 4);
    [gimg(:,:,1) gimg(:,:,3)] = cvtGaborFilter(img, , , , ,);
    [gimg(:,:,2) gimg(:,:,4)] = cvtGaborFilter(img, , , , ,);
end

C = gimg(1+s:h-s,1+s:w-s, :);
X = zeros(8,h-2*s,w-2*s);
X(1,:,:) = gimg(1+s  :h-s  ,1+s+s:w-s+s,1) - C(:,:,1);
X(2,:,:) = gimg(1+s-s:h-s-s,1+s+s:w-s+s,2) - C(:,:,2);
X(3,:,:) = gimg(1+s-s:h-s-s,1+s  :w-s  ,3) - C(:,:,3);
X(4,:,:) = gimg(1+s-s:h-s-s,1+s-s:w-s-s,4) - C(:,:,4);
X(5,:,:) = gimg(1+s  :h-s  ,1+s-s:w-s-s,1) - C(:,:,1);
X(6,:,:) = gimg(1+s+s:h-s+s,1+s-s:w-s-s,2) - C(:,:,2);
X(7,:,:) = gimg(1+s+s:h-s+s,1+s  :w-s  ,3) - C(:,:,3);
X(8,:,:) = gimg(1+s+s:h-s+s,1+s+s:w-s+s,4) - C(:,:,4);
X=double(X>0);
