...
void GBCC(int M, int N, double* Labels, double* Data, double* W, int minleaf, int num_labels, double* bcvar, double* bcval){
    ...
    bh = 0;
    for(nl=0;nl<num_labels;nl++){
        if (diff_labels[nl]>0) bh-=diff_labels[nl]*(log((double)diff_labels[nl])-log((double)sum_W));
    }
    ...
    for(i = 0;i<N;i++){
        ...
        for(j = minleaf-1;j<M-minleaf;j++){
            ...
            for(nl=0;nl<num_labels;nl++) {
                if(diff_labels_l[nl]>0) ch-=(diff_labels_l[nl])*(log((double)diff_labels_l[nl])-log((double)sum_l));
                if(diff_labels_r[nl]>0) ch-=(diff_labels_r[nl])*(log((double)diff_labels_r[nl])-log((double)sum_W-sum_l));
            }
